...
정리는 모르거나 애매하게 알고 있는 것 위주로 정리했고 알고 있는 것이라도 중요하다고 생각하면 간단하게 정리만 했습니다. 정리하는 내용은 강의 내용을 정리했고, 강의에서 다루지 않는 알고리즘도 추가해서 정리할 예정입니다.
...
- 이잔탐색트리란? 이진트리이고, 각 노드에 하나의 키를 저장하며, 각 노드의 왼쪽 자식 노드는 자신보다 작고, 오른쪽 자식 노드는 자신보다 커야 한다.

- 이진탐색트리의 검색 시간 복잡도 
    - 최선 : O(h) // 한 번 탐색할 때마다 1계층씩 증가하므로
    - 최악 : O(n) // 편향된 트리의 경우
...
그래프란
- 노드와 다른 노드를 연결하는 간선을 하나로 모아놓은 자료구조
- 연결되어 있는 객체 간의 관계를 표현할 수 있는 자료구조

#### 그래프 탐색 방법

BFS (Breadth-First Search, 너비우선탐색)
- 노드 간의 최단 경로를 찾을 때 사용

DFS (Depth-First Search, 깊이우선탐색)
- 미로 찾기

최소비용 신장 트리 (MST, Minimum Spanning Tree)
- 최소비용 신장 트리란? 가장 적은 비용으로 노드끼리 연결된 트리
- 그래프 내 최소비용 신장 트리 찾는 방법
    - Kruskal 알고리즘
    - Prim 알고리즘
