...
> 좋은 테스트 코드란, 테스트 대상이 `명확`하고, 결과를 신뢰할 수 있으며, 오류 발생시 쉽게 원인을 찾을 수 있는 가독성을 가진 코드이다.
...
보시다시피 단순히 '자기소개서'라는 Entity를 저장하는 테스트입니다. requestDto에 존재하는 데이터가 DB에 저장된 데이터와 일치하는지 테스트하고 있습니다. 특히 자기소개서안에 문항, 문항 안에 태그가 있는데 이를 함께 검증하고 있습니다.

처음에는 별 문제가 없는 테스트라고 생각했습니다. 하지만 이 테스트를 하려는 목적이 무엇일까? 라고 자문했을 때는 아래와 같은 결론에 도달했습니다.

> 자기소개서를 저장했을 때, 자기소개서 정보와 자기소개서 Entity가 참조하고 있는 문항, 문항이 참조하고 있는 hashtag가 RequestDto와 동일하게 저장되어야 한다.

위 결론을 토대로 테스트 케이스를 재구성했더니 아래와 같은 결과가 나왔습니다.
...
네이밍이나 테스트 로직들을 잘 작성했는지는 모르겠습니다. 그러나 단순히 '자기소개서를 저장`하는 테스트 케이스보다 위와 같이 명확한 목표나 대상을 설정하는 것이 개발자의 의도를 표현하는데 더 도움이 된다는 사실을 알 수 있습니다. 테스트가 실패하더라도 메소드명으로 쉽게 유추할 수도 있겠네요! 
